...
* 📁 **sql/**
  * `01_schema_setup.sql`: Database initialization, relational table constraints, and ingestion health checks.
  * `02_rfm_customer_segmentation.sql`: RFM metric scoring and quartile ranking using window functions.
  * `03_delivery_and_churn_analysis.sql`: Freight costs, delivery delay metrics, and customer churn drivers.

* 📁 **python/**
  * `load_data.py`: Automated batch ETL pipeline inserting 443k+ records into MySQL in seconds.
...
* 📁 **docs/**
  * `project_architecture.md`: System design blueprints and data dictionary.
